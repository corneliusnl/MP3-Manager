...
local ($item_dir, $item_num, $item_name, $item_size, $item_bgcolor, $item_len);
local ($item_totalseconds, $item_mp3version, $item_stereo, $item_mpeglayer, $item_bitrate, $item_copyrighted, $item_frequency);
...
$outfile = "mp3report.htm";
$dirs[@dirs] = 'C:\Audio\Napster\Music';
$dirs[@dirs] = 'C:\Comm\Xircon\Stuph\_mp3';
$dirs[@dirs] = 'C:\Albums';
$dirs[@dirs] = 'D:\Albums';
$dirs[@dirs] = 'D:\symphonic iNSANiTY!\_dump';
$dirs[@dirs] = 'D:\symphonic iNSANiTY!\Insanity';
...
if ($longopt_id3v2) {
  ...
  foreach $tag (keys %$MPEG::MP3Info::v2_tag_names) {
...
sub dodir {
  ...
  foreach $mp3 (@files) {
    ...
    if (my $mp3info = get_mp3info($dir.$dirsep.$mp3)) {
      ...
      # clear out id3v2 variabled from previous match
      foreach $tag (keys %MPEG::MP3Info::v2_tag_names) {
      ...
      $report_item =~ s/\$(\w+)/${$1}/g;
      print TMP $report_item;
    }
    else { print STDERR "\nWarning: $dir$dirsep$mp3 is not a valid MP3, skipping..."; }
  }
  closedir(DIR);
  if (@files) { print TMP $template_itemfooter; }
...
$template_itemdata = <<'KEGS';
        <TR>
          ...
          <TD WIDTH="90%" BGCOLOR="$item_bgcolor"><FONT SIZE="$t_fontsize" FACE="Verdana,Tahoma,Arial,sans-serif">$item_name</FONT></TD>
...
=head1 INSTALLATION
 ...
 use it, it is available at http://search.cpan.org/search?dist=MPEG-MP3Info.
...
http://search.cpan.org/search?dist=MPEG-MP3Info
...
http://www.bdc.cx/software/mp3html/
(a similar type program written in C -- very nice)
